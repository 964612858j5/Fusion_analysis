...
def run_cellpose_process(args, result_queue, stop_flag):
    try:
        ...
        loader = OMETIFFLoader(args["ome_path"], args.get("name_map") or {})
        fusion = FusionEngine()
        tasks = args["tasks"]
        groups = args["groups"]
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # FIX: allow direct step navigation
        for lbl, handler in (
        ...
        # FIX: Next button now means sequential progression only
    ...
    def _go_to_step2(self):
        ...
        if self.is_sequential_flow and self.step1_output:
            ...
            rois = self.step1_output.get("roi_info")
            if rois:
