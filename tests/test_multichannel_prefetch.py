...
def _make(channels=("c0", "c1", "c2", "c3", "c4"), cancel_callbacks=True,
          controller_cls=MultiChannelPrefetchController, **ctl_kwargs):
    """The PRODUCTION controller: HOT only, no COVERAGE to opt out of.

    `controller_cls` exists so the experimental COVERAGE suite can reuse
    this fixture verbatim with its own subclass; nothing in this module
    passes it.
    """
...
# ── production surface: HOT only ─────────────────────────────────────────────

def test_production_controller_has_no_coverage_surface(app):
    """COVERAGE lives in `viewer.experimental.coverage_prefetch` now.

    The production class must not carry the flag, the stats or the state --
    a leftover `coverage=False` parameter would let a caller believe the
    feature is one keyword away, and a leftover stats key would make a
    consumer think it ran.
    """
    controller, scheduler, hot = _make()
    try:
        with pytest.raises(TypeError):
            MultiChannelPrefetchController(
                FakeController(("c0", "c1")), FakeScheduler(),
                [ChannelCorrectionSpec("c0", 9, 13)],
                TileGridSpec(tile_size=16), coverage=True)

        assert not hasattr(hot, "coverage_enabled")
        assert not hasattr(hot, "coverage_inflight")
        assert [k for k in hot.stats if "coverage" in k] == []
        assert [a for a in vars(hot) if "coverage" in a] == []
        assert not hasattr(type(hot), "_coverage_tile_delivered")
    finally:
        hot.stop()


def test_lifecycle_hooks_are_no_ops_on_the_production_controller(app):
    """The four hooks exist so the experiment can act; on their own they
    must do nothing at all -- no requests, no cancellations."""
    controller, scheduler, hot = _make()
    try:
        _ready_overviews(controller)
        before_requests = len(scheduler.requests)
        before_cancels = len(scheduler.cancelled_generations)

        hot._after_settle(_snapshot(controller))
        hot._after_abort()
        hot._after_hot_activity()
        hot._on_stop()

        assert len(scheduler.requests) == before_requests
        assert len(scheduler.cancelled_generations) == before_cancels
    finally:
        hot.stop()


def test_hot_request_sequence_matches_the_pre_extraction_golden(app):
    """Byte-for-byte guard that pulling COVERAGE out did not disturb HOT.

    `tests/data/hot_request_sequence_golden.json` was GENERATED by running
    this same fixture against the code before the extraction (commit
    a79187f) -- it is a recording, not a hand-written expectation. It pins
    ...
    for centre, expected in sorted(golden.items()):
        ...
        try:
            _ready_overviews(controller)
            controller.gesture_quiet.emit(
                _snapshot(controller, channel=centre,
                          visible=((0, 0), (1, 0))))
            _fire_confirm(hot)
            _drain_requests(scheduler)

            actual = [{
                "channel": r["request"].key.channel,
                "method": r["request"].key.method,
                "params": list(r["request"].key.params),
                "level": r["request"].key.tile.level,
                "tx": r["request"].key.tile.tx,
                "ty": r["request"].key.tile.ty,
