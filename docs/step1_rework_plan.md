...
- **专项门**：新增 `tests/test_step0_authoritative_save_barrier.py`（12 passed）：真实 GeometryPersistWorker + 真实临时 handoff 文件覆盖 ROI block → 完整 Save → 实际 `_go_to_step1()` gate 放行；三类 Save 失败；迟到旧失败无 reblock/invalidation；旧发布等待并被 supersede；Save 后 N+1 编辑再锁再恢复；dataset 隔离；无 worker 兼容；`_emit_complete` 成功时 adopt 在 emit 前、失败不 adopt；并加入真实 MainWindow completion/load/entry 轨迹。
...
### C.4.5d.1 新 ROI 完整 Save 后的 MainWindow 重绑（2026-09-19，真机待验收）

- **测量结论**：原拒绝层级在旧 geometry-only invalidation 到达 MainWindow 时暴露：处理器此前只比较 manifest 路径，迟到旧 ROI 的 `roi_changed` 仍可清除新 Save 的 Step1 状态。新 payload 携带 `dataset_gen`、`roi_id`、`roi_dir`、`geometry_revision`；处理器在精确 manifest 绑定之后再做 generation、ROI 身份和 revision 单调性检查。
- **生产轨迹门**：`test_same_window_full_save_rebinds_mainwindow_and_reenters_step1` 使用真实 `MainWindow`、真实 `Step0Page._save_and_continue()`、真实 `step0_complete → _on_step0_complete() → _load_step0_roi_result(auto=True)`、真实 `_go_to_step1()` 与 Qt event drain；未直接设置 `_step1_context_ready`，也未使用 `_entry_probe()`。轨迹覆盖首次 Save 放行、返回 Step0 后新 ROI 的 geometry-only 拒绝、第二次完整 Save 铸造新 context/manifest、真实 handoff 读取、立即重新进入 Step1，并确认排空 Qt 后仍停留在 Step1。
- **失败门**：corrected product、handoff、manifest 任一失败不 adopt；未 Save、新 ROI geometry-only block、Save 后再次编辑、跨 dataset authority、迟到旧 invalidation、旧 context fallback 与绕过 geometry gate 均保持 fail-closed。旧 invalidation 只有在 generation、ROI identity、manifest identity 与 revision 均不冲突时才可使当前 context 失效。
- **专项与变异门**：在最终提交前运行 `test_step0_authoritative_save_barrier.py`、handoff/save-only/invalidation、geometry/patch/background-output 及 C4.5a/b 相关套件；变异点包括移除真实 completion reader、提前设置 `_step1_context_ready`、放宽旧 invalidation identity、Save 前 adopt、绕过 `_go_to_step1()` geometry gate，任一均应使轨迹门失败。
- **真机验收仍待用户执行**：初次完整 Save 后进入 Step1；返回 Step0 画新 ROI，未 Save 时确认立即拒绝；完整 Save 成功后不等待重启、立即进入 Step1；再制造 Save 失败仍拒绝；Save 后再次编辑仍拒绝，直到对应新 geometry 发布或再次完整 Save。
