...
Each panel holds ONE in-memory array with `levels=(min, max)` and a lookup
table carrying gamma and colour; the nucleus is a second item ADDED on top
(CompositionMode_Plus) under its own channel's mapping. Per pixel that is

    clip(marker_colour * clip((m-min)/(max-min))**gamma
         + nucleus_colour * clip((n-min)/(max-min))**gamma)

which is asserted here against the pixels actually painted, by grabbing the
widget -- not against a second implementation of the sum, because there is
no second implementation left to disagree with.

What feeds the panels is the VIRTUAL PATCH: the arrays a
`CompareRegionWorker` produced for the region the user right-clicked.
`_apply_compare_payload` is the one door they come through, and the tests
use it directly -- no slide, no provider, no thread.
...
def _page(app):
    ...
    page.show()
    # The panels are the second page of the viewing-area stack; they must be
    # the visible one for `grab()` to have anything to say.
...
def _payload(marker=500.0, nucleus=300.0, h=64, w=64, with_nucleus=True,
    ...
    """Raw-intensity payload the way `CompareRegionWorker` builds it."""
    ...
    return {"original_raw": m, "tophat_raw": m * tophat_scale,
            ...
            "nucleus_raw": (np.full((h, w), nucleus, np.float32)
                            if with_nucleus else None)}


def _show(page, payload, *, refit=True):
    """Put a virtual patch on the panels, as a worker reply would."""
    if refit:
...
def _expected(page, marker, nucleus, m_map, n_map, marker_on=True,
              nucleus_on=True):
    """The colour the panel must paint: the marker's mapped value in the
    channel's colour, plus the nucleus's in its own, saturating."""
    marker_rgb = np.asarray(page._channel_color(page.current_channel), float)
    nuc_rgb = np.asarray(page._channel_color(page.nucleus_channel), float)
    out = marker_rgb * (_mapped(marker, *m_map) if marker_on else 0.0)
    if nucleus_on and nucleus is not None:
        out = out + nuc_rgb * _mapped(nucleus, *n_map)
...
def test_the_painted_pixel_is_the_mapping_of_the_raw_value(app):
    ...
    want = _expected(page, 500.0, 400.0, (100.0, 900.0, 1.0),
                     (0.0, 1000.0, 1.0))
...
def test_gamma_and_range_changes_are_table_and_level_swaps(app):
    ...
    assert np.allclose(after, _expected(page, 400.0, 200.0,
                                        (0.0, 500.0, 0.5), (0.0, 400.0, 1.0)),
                       atol=2 / 255)
    # No per-pixel work: the item still holds the same raw buffer.
    assert np.shares_memory(page._preview_imgs[0].image, image_before), (
        "the pixels were rebuilt")
...
def test_the_seed_comes_from_the_payload_when_the_slide_cannot_be_read(app):
    """A loader without a pyramid (the fakes) cannot seed from the slide, so
    the virtual patch's own raw pixels seed the mapping -- QuPath-style
...
def test_the_switches_hide_and_show_each_layer(app):
    ...
    QtTest.QTest.qWait(30)
    assert np.allclose(_centre_pixel(page, 0),
                       _expected(page, 600.0, 400.0, *maps, marker_on=False),
                       atol=2 / 255)
...
def test_the_nucleus_is_never_added_to_itself(app):
    """Selecting the DAPI row makes the MARKER layer the nucleus channel.
...
def test_legacy_normalised_payloads_still_display(app):
    ...
    m = {"snr": 1.0, "bg_cv": 0.1}
    _show(page, {"original_disp": disp, "tophat_disp": disp,
                 "cucim_disp": disp, "original_metrics": m,
...
def test_nucleus_items_are_created_on_first_use_additive_and_row_major(app):
    ...
    assert page._preview_nuc_imgs[1] is not None
...
def test_the_marker_items_are_row_major_at_the_origin(app):
    """axisOrder EXPLICITLY, never the process-global: "Save as patch"
    converts these coordinates to level-0, so a silent transposition would
    record the wrong part of the slide."""
    page = _page(app)
    _show(page, _payload(h=40, w=90))
    for item in page._preview_imgs:
        assert item.axisOrder == "row-major"
        rect = item.boundingRect()
        assert (rect.x(), rect.y()) == (0.0, 0.0)
        assert (rect.width(), rect.height()) == (90.0, 40.0)


def test_the_metrics_are_the_workers_own_numbers(app):
    page = _page(app)
    payload = _payload()
    payload["tophat_metrics"] = {"snr": 9.5, "bg_cv": 0.42}
    _show(page, payload)

    assert "9.5" in page._metrics_tophat.text()
    assert "0.42" in page._metrics_tophat.text()
    assert "(preview)" in page._metrics_tophat.text()
