"""G3.2b.4B2: why a first cold Patch takes ~2 s when a cold Tissue landing
takes under 1 s. READ-ONLY DIAGNOSTIC -- it changes no production behaviour.
...
CHANNEL = "CD22"                      # Step0 decision `cucim`, real product
NUCLEUS = "DAPI"
...
def build_window(ledger):
    ...
    with state.using_scope(STEP1_SCOPE):
        for name in (CHANNEL, NUCLEUS):
            state.set_display_visible(name, name == CHANNEL)
            state.set_mapping(name, 0.0, 30.0, 1.0)
        state.set_selected_channel(CHANNEL)
    domain = mount._domain
    domain.initialize_dataset({"markers": {CHANNEL: 1.0}}, NUCLEUS,
                              channels=names)
    domain.set_fusion_enabled(CHANNEL, True)
    domain.edit_channel_weight(CHANNEL, 1.0)
    ...
    assert wait_for(lambda: CHANNEL in mount.gpu_binding.stats()["coarse_channels"],
                    600.0), "the channel's complete coarse never landed"
    drain(800)
...
def measure(window, mount, ledger, heart, paints, work, label, kind, arg,
    ...
    def ready():
        return binding._viewport_fine_ready(CHANNEL)
...
def main():
    out = pathlib.Path(sys.argv[1] if len(sys.argv) > 1
                       else "patch_vs_tissue.json")
    label = sys.argv[2] if len(sys.argv) > 2 else "g324b2"
    ledger = Ledger()
    ...
    report = {
        ...
        "channel": CHANNEL,
        "source_of_channel": table.source_of(CHANNEL) if table else "?",
        "backend": mount.backend,
        "gpu": mount.gpu_status().get("environment", {}),
        "patches": [list(b) for b in window._all_patches],
        "cold_definition": (
            "a region of the slide this session has never visited "
            "(new-region first product access / in-process cold path); the OS "
            "page cache was NOT dropped and no number here is a cold disk read"),
        "honesty": [
            ...
            "natural paints are Qt presentation events, not photons",
            "the demo project is read-only; the session autosave is left ON "
            "but writes to a scratch directory",
        ],
        "A_natural": [], "B1_same_centre": [], "B2_matched_viewport": [],
        "C_zoom_sweep": [],
    }

    # ── A. the natural product paths, alternating, from ONE parked camera ─
    park(mount)
    report["park"] = {"y": HOME_Y, "x": HOME_X, "size": HOME_SIZE,
                      "camera": _camera(mount)}
    # Alternating, and EVERY target distinct: five patches never clicked
    # before, five points never landed on before.
    ...
    for index, (kind, slot) in enumerate(order):
        ...
        name = f"A{index + 1}-{kind}-{slot}"
        park(mount)
        report["A_natural"].append(
            measure(window, mount, ledger, heart, paints, work, name, kind,
                    arg, table))
        drain(400)

    # ── B1. the SAME centre, each entry keeping its own semantics ────
    # Both land on the middle of patch P1; the patch fits the whole bbox,
    # the tissue keeps the parked zoom. Never visited before this point.
    # Two DIFFERENT never-visited patches under identical conditions: the
    # tissue run lands on one patch's centre keeping the parked zoom, the
    # patch run clicks the other and fits its whole bbox.
    ty0, ty1, tx0, tx1 = window._all_patches[5]
    park(mount)
    report["B1_same_centre"].append(
        measure(window, mount, ledger, heart, paints, work, "B1-tissue",
                "tissue", ((ty0 + ty1) // 2, (tx0 + tx1) // 2), table))
    park(mount)
    report["B1_same_centre"].append(
        measure(window, mount, ledger, heart, paints, work, "B1-patch",
                "patch", 6, table))

    # ── B2. the SAME final viewport, reached both ways, both cold ────
    # The patch entry decides its own fitted rectangle; the tissue entry is
    # then parked AT THAT SCALE and sent to the same centre, on a patch
    # neither has visited.
    park(mount)
    patch_run = measure(window, mount, ledger, heart, paints, work,
                        "B2-patch", "patch", 7, table)
    fitted = patch_run["camera_after"]
    # Park at the FITTED scale, far away, then land on a DIFFERENT
    # never-visited patch's centre: same final rectangle, same cold state.
    ...
    report["B2_matched_viewport"] = [patch_run, tissue_run]

    # ── C. how the gap scales with the zoom the user is working at ───
    # A Tissue landing KEEPS the current viewport; a Patch click replaces it
    # with the patch's own rectangle. So the further the user is zoomed in,
    # the more the patch entry has to widen the view -- and the bigger the
    # gap should be. This is the sweep that says by how much.
    for index, zoom in enumerate(SWEEP_ZOOMS):
        park(mount, size=zoom)
        parked = _camera(mount)
        tissue = measure(window, mount, ledger, heart, paints, work,
                         f"C{index + 1}-tissue-zoom{zoom}", "tissue",
                         SWEEP_POINTS[index], table)
        park(mount, size=zoom)
        patch = measure(window, mount, ledger, heart, paints, work,
                        f"C{index + 1}-patch-zoom{zoom}", "patch",
                        9 + index, table)
        report["C_zoom_sweep"].append(
            {"zoom_short_side_l0": zoom, "parked_camera": parked,
             "tissue": tissue, "patch": patch})

    heart.stop()
    work.restore()
    report["cufile_note"] = "run from /tmp; the repo's cufile.log is untouched"
    out.write_text(json.dumps(report, indent=2, default=str))
    print(f"wrote {out}")
    try:
        window._display.shutdown("g324b2")
        window.deleteLater()
        APP.processEvents()
    except Exception:                                       # noqa: BLE001
        pass
