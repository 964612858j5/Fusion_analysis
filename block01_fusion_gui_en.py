...
class FusionEngine:
    ...
    def compute(self, cache, groups, group_weights, nuc_ch, nuc_w):
        ...
        if nuc_ch and nuc_ch in cache:
            nucleus = cache[nuc_ch] * float(nuc_w)
            mx = nucleus.max()
            if mx > 0:
                nucleus /= mx
...
class ConfigPanel(QWidget):
    ...
    def load_panel(self, groups, nuc_ch):
        ...
        for gname, channels in groups.items():
            self._add_group(gname, channels)
...
class MainWindow(QMainWindow):
    ...
    def _parse_panel_csv(path):
        """
        Parse a panel CSV with columns: marker, group
        - group='nucleus' → nucleus channel
        - other groups → cyto fusion groups (weight=1.0)
        ...
        with open(path, newline='', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                marker = row.get('marker') or row.get('channel') or row.get('name', '')
                group  = row.get('group') or row.get('category') or row.get('class', '')
                if not marker:
                    continue
                if group.lower() in ('nucleus', 'dapi', 'nuclear'):
                    nuc_ch = marker
                elif group:
                    groups.setdefault(group, {})[marker] = 1.0
        return groups, nuc_ch
        ...
        if panel_csv and os.path.exists(panel_csv):
            ...
            try:
                ...
                QMessageBox.information(
                    ...
                    f'Please open the file, fill in the "group" column\n'
                    f'(use "nucleus" for DAPI, any name for other groups),\n'
    ...
    def _render_current_patch(self, reset_view=False):
        ...
        if not cache:
            return
        cyto, nuc = self.fusion.compute(
            cache,
            self.config.get_groups(),
            self.config.get_group_weights(),
            *self.config.get_nucleus(),
        )
        if cyto is None:
            return
